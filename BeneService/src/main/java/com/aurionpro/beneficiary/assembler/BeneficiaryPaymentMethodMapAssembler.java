...
public class BeneficiaryPaymentMethodMapAssembler extends MakerCheckerDetailsAssembler {
	 ...
	 */
	@Override
	public BeneficiaryPaymentMethodMapBO assemble(Ticket ticket,BaseBO bo, BaseDetails details) throws BaseException, RemoteException {
		...
		beneficiaryPaymentMethodMapBO.setCorporateId(EncryptUtils.decryptId(ticket.getCurrentOUId()));
		beneficiaryPaymentMethodMapBO.setCorporateUserId(EncryptUtils.decryptId(ticket.getUserId()));
		
		if (ticket.getUserType().equals(RequestBy.CORPORATE.name())) {
			beneficiaryPaymentMethodMapBO.setRequestBy(RequestBy.CORPORATE.name());
		} else {
			beneficiaryPaymentMethodMapBO.setRequestBy(RequestBy.BANK.name());
		}
